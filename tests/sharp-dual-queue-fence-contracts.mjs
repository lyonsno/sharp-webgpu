...
await withFakeClock(async setNow => {
  ...
  for (const contradictoryRange of [
    ...
    },
    {
      ...
      completedAtNs: '1000000',
      rawDurationNs: '0',
      durationNs: 'not-a-number',
      durationMs: 1,
      resolutionUpperBoundNs: 'not-a-number',
    },
    {
      schema: 'sharp-webgpu.gpu-timestamp-range.v0',
      authority: 'timestamp-query-inside-submitted-command-buffer',
    ...
    assert.throws(
      ...
      /contradictory resolution-censored GPU timing|invalid GPU timestamp-query measurement record/,
      'forged or incomplete GPU timing metadata must not enter the adaptive planner',
    );
  }
});
